...
**Benefits:**
- **3-5x faster training**: Spectrograms are computed once and loaded from disk
...
```bash
# Preprocess all audio files (takes 5-10 minutes)
python scripts/preprocess_dataset.py --output data/processed/v12
```

This will:
1. Load all audio files from `data/`
2. Compute mel spectrograms
3. Save as `.npy` files in `data/processed/v12/<hash>/`
4. Create a manifest for fast loading

**Expected output:**
```
Found 8695 raw audio files (~8,264 after MIN_AUDIO_DURATION filtering)
Preprocessing config hash: f8494771d0d9d2bd
Processing audio files...
100%|████████████████| 8695/8695 [04:50<00:00, 29.9it/s]
...
Average time per file: 0.033 seconds
Total storage used: ~408 MB
...
Edit [[Sound-Localization-Claude/baby_cry_detection/src/config.py|config.py]]:

```python
# Preprocessed data configuration
USE_PREPROCESSED = True  # Changed from False
PREPROCESSED_DIR = Path("data/processed/v12")  # Matches --output above
```
...
You should see:
```
Using preprocessed data from: data/processed/v12
Found preprocessed directory: data/processed/v12/f8494771d0d9d2bd
Loaded manifest with 8695 files
```
...
```bash
python scripts/preprocess_dataset.py --output data/processed/v12 --dry-run
```
...
```bash
python scripts/preprocess_dataset.py --output data/processed/v12 --overwrite
```
...
```bash
python scripts/preprocess_dataset.py --validate data/processed/v12/f8494771d0d9d2bd
```
...
**For 8,695 files (after MIN_AUDIO_DURATION filtering):**
- Original audio: ~50-100 MB (compressed)
- Preprocessed data: **~408 MB** (uncompressed)

**Total overhead: ~320-360 MB**
...
> Make sure you ran the preprocessing script first:
> ```bash
> python scripts/preprocess_dataset.py --output data/processed/v12
> ```

> [!warning]- Config hash mismatch
...
> ```bash
> python scripts/preprocess_dataset.py --output data/processed/v13
> ```
>
> Then update config:
> ```python
> PREPROCESSED_DIR = Path("data/processed/v13")
> ```

> [!warning]- Training is slower with preprocessing
> **Possible causes:**
...
> ```bash
> rm -rf data/processed/v12/old_hash/
> ```
>
> **Solution 2:** Preprocess to external drive:
> ```bash
> python scripts/preprocess_dataset.py --output /path/to/external/drive/processed/v12
> ```
>
> Then update config:
> ```python
> PREPROCESSED_DIR = Path("/path/to/external/drive/processed/v12")
> ```
...
[[Sound-Localization-Claude/baby_cry_detection/src/config.py|config.py]]:
```python
USE_PREPROCESSED = True
PREPROCESSED_DIR = Path("data/processed/v12")
```
...
```bash
# Windows
rmdir /s data\processed\v12\old_hash
...
- [[Sound-Localization-Claude/baby_cry_detection/QUICK_START|Quick Start]] - Getting started tutorial
- [[Sound-Localization-Claude/baby_cry_detection/README|Baby Cry Detection]] - Project overview

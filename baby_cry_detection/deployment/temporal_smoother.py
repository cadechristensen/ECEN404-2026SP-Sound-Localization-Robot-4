...
class TemporalSmoothedDetector:
    ...
    Usage:
        ...
        for audio_chunk in audio_stream:
            is_cry, confidence, gate_rejected = detector.detect_cry(audio_chunk)
    ...
    def __init__(
        ...
        max_prediction_age: float = 3.0,
        confidence_stability_threshold: float = 0.15
    ):
        """
        ...
        Args:
            ...
            min_consecutive: Minimum number of consecutive high-confidence
                             predictions required before alerting. Default 4.
            confidence_threshold: Threshold above which a prediction is
                                  considered high-confidence. Default 0.5.
            confidence_stability_threshold: Maximum standard deviation allowed
                                  across the consecutive window. Babbling produces
                                  fluctuating confidence (high std), while real cries
                                  produce consistently high confidence (low std). Default 0.15.
        ...
        logging.info(
            ...
            f"stability_threshold={confidence_stability_threshold}"
        )
    ...
    def update(self, confidence: float, timestamp: Optional[float] = None) -> SmoothedDetectionResult:
        ...
        .. note::
            ``should_alert`` is ``True`` on every call for as long as the
            ``min_consecutive`` streak is maintained — there is no built-in
            one-shot latch.  Callers **must** call :meth:`reset` after acting
            on an alert (or enforce their own cooldown) to prevent the same
            event from triggering repeated wake-ups.
        """
        ...
        with self._lock:
            ...
            # Check confidence stability across the consecutive window
            confidence_std = self._check_confidence_stability()
            is_stable = confidence_std < self.confidence_stability_threshold

            # Determine if we should alert: both consecutive count AND stability must pass.
            # Baby babbling sustains 2+ seconds but with fluctuating confidence (high std),
            # while real cries produce consistently high confidence (low std).
            should_alert = consecutive_count >= self.min_consecutive and is_stable

            if consecutive_count >= self.min_consecutive and not is_stable:
                logging.info(
                    f"Temporal smoothing: consecutive count met ({consecutive_count}) "
                    f"but confidence unstable (std={confidence_std:.3f} >= "
                    f"{self.confidence_stability_threshold}) — suppressing alert"
                )

            # Get confidence history for debugging
            history = [p.confidence for p in self._predictions]

        # Use len(history) — the snapshot taken inside the lock — rather than
        # re-reading len(self._predictions) here (outside the lock), which could
        # be 0 if reset() was called concurrently between the lock release and
        # this line, producing an inconsistent result object.
        result = SmoothedDetectionResult(
            ...
            predictions_in_window=len(history),
    ...
    def _count_consecutive_high(self) -> int:
        ...
        ignoring any prediction older than max_prediction_age seconds.
        ...
        if not self._predictions:
            return 0
        # Use the most recent prediction's timestamp as reference, not
        # time.time(). Callers may pass audio-relative timestamps (e.g.
        # offline test scripts) where wall-clock time would make every
        # prediction appear billions of seconds old.
        now = self._predictions[-1].timestamp
        count = 0
        for pred in reversed(self._predictions):
            if now - pred.timestamp > self.max_prediction_age:
                break
            if pred.is_positive:
                count += 1
            else:
    ...
    def _check_confidence_stability(self) -> float:
        """
        Compute standard deviation of the last min_consecutive predictions.

        Baby babbling produces fluctuating confidence (some chunks cry-like,
        others not), while real cries produce consistently high confidence.
        A low std indicates stable, trustworthy predictions.

        Returns:
            Standard deviation of the most recent min_consecutive predictions,
            or 0.0 if not enough predictions yet.
        """
        if len(self._predictions) < self.min_consecutive:
            return 0.0
        recent = [p.confidence for p in list(self._predictions)[-self.min_consecutive:]]
    ...
    def get_state_summary(self) -> Dict:
        ...
        """
        with self._lock:
            preds = list(self._predictions)          # snapshot under lock
            consecutive = self._count_consecutive_high()
            smoothed = self._calculate_smoothed_confidence()
            return {
                'window_size': self.window_size,
                ...
                'is_window_filled': len(preds) >= self.window_size,
                'recent_confidences': [p.confidence for p in preds],
            }

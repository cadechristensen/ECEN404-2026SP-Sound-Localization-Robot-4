...
class ChannelHealthMonitor:
    ...
    def get_channel_health(self, audio: np.ndarray) -> List[ChannelQualityMetrics]:
        ...
        coherence_dict = self.check_coherence(audio)

        # Compute SNR
        snr_computer = EnhancedSNRComputation(sample_rate=self.sample_rate)
        ...
        for ch in range(len(rms_values)):
            ...
            ch_coherence = [
                ...
                if f"ch{ch}-" in key or f"-ch{ch}" in key.replace(f"ch{ch}-", "")
            ]
...
class DualChannelVotingDetector:
    ...
    def __init__(
        ...
        self.health_monitor = ChannelHealthMonitor(sample_rate=sample_rate, num_channels=num_channels)
    ...
    def select_best_channels(self, audio: np.ndarray, n_channels: int = 2) -> Tuple[List[int], np.ndarray]:
        ...
        n_channels = min(n_channels, len(sorted_indices))
        best_channels = sorted_indices[:n_channels].tolist()
    ...
    def detect_cry_dual_channel(
        ...
        for ch in best_channels:
            ...
            # Run detection
            is_cry_ch, confidence_ch = self.detector.detect_cry(channel_audio, use_tta=use_tta)
    ...
    def detect_with_health_check(
        ...
        Args:
            audio: Multi-channel audio (num_samples, num_channels)
            use_tta: Whether to use test-time augmentation
            confidence_threshold: Confirmation threshold for wake-up (default: 0.85)

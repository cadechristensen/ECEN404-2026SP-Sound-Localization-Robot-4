...
#python C:\Users\Cade\Downloads\AppMobileFire.py
#IP: 165.91.10.243
import os
import socket
import threading
...
@app.route('/status')
def status():
    """Endpoint for the Android app to poll for notification status."""
    return jsonify(notification_status)


# This is a simple test route to trigger a notification.
# In a real application, you'd call this from your baby detection logic.
@app.route('/trigger_notification')
def trigger_notification():
    """Sets the notification status to true."""
    global notification_status
    score = 0.95 # Example score
    notification_status["should_notify"] = True
    notification_status["confidence_score"] = score
    notification_status["message"] = f"BABY MONITOR ALERT! Confidence : {score}"
    return "Notification triggered!"

@app.route('/reset_notification')
def reset_notification():
    """Resets the notification status to false."""
    global notification_status
    notification_status["should_notify"] = False
    notification_status["confidence_score"] = 0.0
    notification_status["message"] = "BABY MONITOR ALERT"

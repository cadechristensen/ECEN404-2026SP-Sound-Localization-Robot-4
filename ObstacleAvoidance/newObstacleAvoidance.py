...
ARRIVAL_THRESHOLD = 1.5  # Stop when within 0.5m of target
...
def parse_uart():
    ...
    if uart.any():
        ...
        if msg.startswith("NAV"):
            ...
            print("Sensors NOW ACTIVE - starting navigation")
...
stop()
print("System ACTIVE - Waiting for navigation command...")
print("Sensors IDLE until NAV command received")
print("-" * 60)
...
while True:
    ...
    # =========================================================================
    # ONLY READ SENSORS IF NAVIGATION IS ACTIVE
    # =========================================================================
    if not nav_active:
        # No navigation command yet - stay idle
        stop()
        time.sleep(0.1)
        continue

    # Navigation is active - start reading sensors
    d = read_distances()
    ...
    print(f"S:{state} | F:{f:.0f} L:{l:.0f} R:{r:.0f} | NAV:{nav_dir}")
    ...
    if not obstacle_encountered:
        ...
        if elapsed >= nav_duration:
            ...
            print("Sensors going IDLE - waiting for next command")
            state = GO_STRAIGHT
            continue
    ...
    if state == GO_STRAIGHT:
        if front_blocked:
            ...
            # No obstacle in front - execute navigation command
            if nav_dir == "FORWARD":
                forward()
            elif nav_dir == "LEFT":
                turn_left()
    ...
    # =========================================================================
        ...
        if front_blocked:
            ...
            obstacle_encountered = False
            state = GO_STRAIGHT
            print("Sensors going IDLE - waiting for RELISTEN response")
        else:

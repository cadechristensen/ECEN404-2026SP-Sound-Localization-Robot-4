...
class DataGenerator(object):
    ...
    def split_multi_channels(data, num_channels):
        ...
        if len(in_shape) == 3:
            # (batch, time, features)
            feature_len = in_shape[2]
            if feature_len % num_channels != 0:
                raise ValueError(f"Feature dimension {feature_len} not divisible by num_channels {num_channels}")
            hop = feature_len // num_channels
            tmp = np.zeros((in_shape[0], num_channels, in_shape[1], hop), dtype=data.dtype)
            for i in range(num_channels):
                tmp[:, i, :, :] = data[:, :, i * hop:(i + 1) * hop]
        elif len(in_shape) == 4 and num_channels == 1:
            tmp = np.zeros((in_shape[0], 1, in_shape[1], in_shape[2], in_shape[3]), dtype=data.dtype)
            tmp[:, 0, :, :, :] = data
        else:
            raise ValueError(f'ERROR: The input should be a 3D matrix (batch,time,features) or 4D with num_channels==1 but got dimensions: {in_shape}')
        return tmp

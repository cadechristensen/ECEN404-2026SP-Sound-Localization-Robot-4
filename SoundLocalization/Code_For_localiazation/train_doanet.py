...
sys.path.insert(0, '/home/sam/CapStone/hungarian-net')
    ...
    with torch.no_grad():
        for data, target in data_generator.generate():
            ...
            if params['use_hnet']:
                ...
                dMOTP_loss = (torch.mul(dist_mat, da_mat).sum(-1).sum(-1) * da_mat.sum(-1).sum(-1)*params['dMOTP_wt']).sum() / (da_mat.sum() + 1e-7)
                M = da_activity.sum(-1)
            ...
            dist_mat_hung = torch.clamp(dist_mat_hung, -1+eps, 1-eps)
            dist_mat_hung = torch.acos(dist_mat_hung)  
...
def main(argv):
    ...
    hnet_model.load_state_dict(torch.load("hungarian-net/data/hnet_model.pt", map_location=torch.device('cpu')))
